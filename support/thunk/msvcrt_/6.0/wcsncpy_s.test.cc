...
TEST_CASE("wcsncpy_s")
{
  ...
  {
    CLEAR_TOUCH_FLAG();
    errno_t err = wcsncpy_s(dest, N, src, src_len);
    REQUIRE_TOUCHED();
  ...
  {
    dest[0] = 1;
    errno_t err = wcsncpy_s(dest, src_len, src, src_len);
    REQUIRE(err == ERANGE);
    REQUIRE(wcscmp(dest, L"") == 0);

    err = wcsncpy_s(dest, src_len + 1, src, src_len);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, src) == 0);

    err = wcsncpy_s(dest, src_len + 1, src, src_len + 1);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, src) == 0);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcsncpy_s(dest, N, nullptr, src_len);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  {
    errno_t err = wcsncpy_s(nullptr, N, src, src_len);
    REQUIRE(err == EINVAL);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcsncpy_s(dest, 0, src, src_len);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
  ...
  SECTION("error: dest size too large")
  {
    ...
    errno_t err = wcsncpy_s(dest, rsize_max, src, src_len);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
  ...
  SECTION("error: src count is 0")
  {
    ...
    errno_t err = wcsncpy_s(dest, N, src, 0);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  SECTION("error: src count too large")
  {
    dest[0] = 1;
    // ISO C semantic (Microsoft say OK)
    errno_t err = wcsncpy_s(dest, N, src, rsize_max);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  SECTION("error: overlap")
  {
    ...
    errno_t err = wcsncpy_s(dest, N, dest, src_len);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
}
