...
TEST_CASE("wcsncat_s")
{
  ...
  SECTION("normal case")
  {
    ...
    CLEAR_TOUCH_FLAG();
    errno_t err = wcsncat_s(dest, N, str2, str2_len);
    REQUIRE_TOUCHED();
  ...
  {
    fill_dest();
    errno_t err = wcsncat_s(dest, str1_len + str2_len, str2, str2_len);
    REQUIRE(err == ERANGE);
    REQUIRE(wcscmp(dest, L"") == 0);

    fill_dest();
    err = wcsncat_s(dest, str1_len + str2_len + 1, str2, str2_len);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, result) == 0);

    fill_dest();
    err = wcsncat_s(dest, str1_len + str2_len + 1, str2, str2_len + 1);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, result) == 0);
  }
  ...
  {
    fill_dest();
    errno_t err = wcsncat_s(dest, N, nullptr, str2_len);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  {
    errno_t err = wcsncat_s(nullptr, N, str2, str2_len);
    REQUIRE(err == EINVAL);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcsncat_s(dest, 0, str2, str2_len);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
  ...
  SECTION("error: dest size too large")
  {
    ...
    errno_t err = wcsncat_s(dest, rsize_max, str2, str2_len);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
  ...
  SECTION("error: src count is 0")
  {
    ...
    errno_t err = wcsncat_s(dest, N, str2, 0);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  SECTION("error: src count too large")
  {
    fill_dest();
    // ISO C semantic (Microsoft say OK)
    errno_t err = wcsncat_s(dest, N, str2, rsize_max);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  SECTION("error: overlap")
  {
    ...
    errno_t err = wcsncat_s(dest, N, dest, str2_len);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
}
