...
namespace mingw_thunk
{
  namespace musl
  {
    constexpr uint32_t C(uint32_t x)
    {
      return x < 2 ? -1 : (R(0x80, 0xc0) | x);
    }

    constexpr uint32_t D(uint32_t x)
    {
      return C(x + 16);
    }

    constexpr uint32_t E(uint32_t x)
    {
      return (x == 0     ? R(0xa0, 0xc0)
              : x == 0xd ? R(0x80, 0xa0)
                         : R(0x80, 0xc0)) |
             (R(0x80, 0xc0) >> 6) | x;
    }

    constexpr uint32_t F(uint32_t x)
    {
      return (x >= 5   ? 0
              : x == 0 ? R(0x90, 0xc0)
              : x == 4 ? R(0x80, 0x90)
