#include "LocaleNameToLCID.h"

#include <thunk/_common.h>
#include <thunk/yy/api-ms-win-core-localization.h>
...
namespace mingw_thunk
{
  ...
  __DEFINE_THUNK(kernel32,
                 ...
                 _In_ DWORD dwFlags)
  {
    __DISPATCH_THUNK_2(LocaleNameToLCID,
                       const auto pfn = try_get_LocaleNameToLCID(),
                       pfn,
                       &f::fallback_LocaleNameToLCID);

    return dllimport_LocaleNameToLCID(lpName, dwFlags);
  }
  ...
  namespace f
  {
                                             ...
                                             _In_ DWORD dwFlags)
    {
      ...
      } else if (internal::StringCompareIgnoreCaseByAscii(
                     lpName,
                     LOCALE_NAME_SYSTEM_DEFAULT,
                     _countof(LOCALE_NAME_SYSTEM_DEFAULT)) == 0) {
        return GetSystemDefaultLCID();
      }

      // Vista虽然不支持
      // LOCALE_ALLOW_NEUTRAL_NAMES，但是我们好人做到底，就直接支持了吧。
      ...
      // Map of locale name to an index in LcidToLocaleNameTable, for Windows
      // XP. Data in this table has been obtained from National Language Support
      // (NLS) API Reference at
      // http://msdn.microsoft.com/en-us/goglobal/bb896001.aspx The table is
      // sorted to improve search performance.
      ...
      while (bottom <= top) {
        ...
        int testIndex = internal::StringCompareIgnoreCaseByAscii(
            lpName,
            LocaleNameToLcidTable[middle].localeName,
