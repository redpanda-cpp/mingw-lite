...
TEST_CASE("wcscat_s")
{
  ...
  SECTION("normal case")
  {
    ...
    CLEAR_TOUCH_FLAG();
    errno_t err = wcscat_s(dest, N, str2);
    REQUIRE_TOUCHED();
  ...
  {
    fill_dest();
    errno_t err = wcscat_s(dest, str1_len + str2_len, str2);
    REQUIRE(err == ERANGE);
    REQUIRE(wcscmp(dest, L"") == 0);

    fill_dest();
    err = wcscat_s(dest, str1_len + str2_len + 1, str2);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, result) == 0);
  }
  ...
  {
    fill_dest();
    errno_t err = wcscat_s(dest, N, nullptr);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  {
    errno_t err = wcscat_s(nullptr, N, str2);
    REQUIRE(err == EINVAL);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcscat_s(dest, 0, str2);
    REQUIRE(err == EINVAL);
  ...
  SECTION("error: dest size too large")
  {
    ...
    errno_t err = wcscat_s(dest, rsize_max, str2);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
}
