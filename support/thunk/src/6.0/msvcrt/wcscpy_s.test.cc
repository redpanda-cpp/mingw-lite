...
TEST_CASE("wcscpy_s")
{
  ...
  {
    CLEAR_TOUCH_FLAG();
    errno_t err = wcscpy_s(dest, N, src);
    REQUIRE_TOUCHED();
  ...
  {
    dest[0] = 1;
    errno_t err = wcscpy_s(dest, src_len, src);
    REQUIRE(err == ERANGE);
    REQUIRE(wcscmp(dest, L"") == 0);

    err = wcscpy_s(dest, src_len + 1, src);
    REQUIRE(err == 0);
    REQUIRE(wcscmp(dest, src) == 0);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcscpy_s(dest, N, nullptr);
    REQUIRE(err == EINVAL);
    REQUIRE(wcscmp(dest, L"") == 0);
  }
  ...
  {
    errno_t err = wcscpy_s(nullptr, N, src);
    REQUIRE(err == EINVAL);
  }
  ...
  {
    dest[0] = 1;
    errno_t err = wcscpy_s(dest, 0, src);
    REQUIRE(err == EINVAL);
  ...
  SECTION("error: dest size too large")
  {
    ...
    errno_t err = wcscpy_s(dest, rsize_max, src);
    REQUIRE(err == EINVAL);
    REQUIRE(dest[0] == 1);
  }
}
